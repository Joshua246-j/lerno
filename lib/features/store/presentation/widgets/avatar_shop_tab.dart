        ...
        return GestureDetector(
          onTap: () {
            ...
            if (avatar.unlockType == 'Coins') {
              ...
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Cannot purchase. Requires: ${avatar.unlockType}')));
            }
          },
          child: Container(
                ...
                Expanded(
                  child: Padding(
                    ...
                    child: SvgPicture.asset(
                      ...
                      placeholderBuilder: (context) => const Icon(Icons.person, size: 50, color: AppTheme.primaryBlue),
                    ),
                  ),
                ),
                Text(avatar.displayName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                const SizedBox(height: 5),
                Text(avatar.rarity, style: const TextStyle(color: AppTheme.primaryBlue, fontSize: 12, fontWeight: FontWeight.bold)),
                const SizedBox(height: 10),
                Container(
                  padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 20),
                  decoration: BoxDecoration(
                    color: isOwned ? Colors.green.shade100 : AppTheme.primaryLight,
                  ...
                  child: Row(
                    ...
                    children: [
                      if (isEquipped) ...[
                        ...
                        const Text('Equipped', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green)),
                      ] else if (isOwned) ...[
                        const Icon(Icons.check, color: Colors.green, size: 16),
                        const SizedBox(width: 4),
                        const Text('Owned', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green)),
                      ] else if (avatar.unlockType == 'Coins') ...[
                        const Icon(Icons.monetization_on, color: Colors.amber, size: 16),
                        const SizedBox(width: 4),
                        Text('${avatar.coinCost}', style: const TextStyle(fontWeight: FontWeight.bold, color: AppTheme.primaryBlue)),
                      ] else ...[
                        const Icon(Icons.lock, color: AppTheme.primaryBlue, size: 16),
                        const SizedBox(width: 4),
                        Expanded(child: Text(avatar.unlockType, style: const TextStyle(fontWeight: FontWeight.bold, color: AppTheme.primaryBlue, fontSize: 10), overflow: TextOverflow.ellipsis)),
                      ],
                    ],
                  ),
                )
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(ctx);
              final success = await ref.read(shopProvider.notifier).purchaseAvatar(avatar.avatarId, cost);
              if (success && context.mounted) {
                ref.read(audioManagerProvider).playSuccess();
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Purchased ${avatar.displayName}!')));
