...
## ☁️ Backend & Services (Legacy / To Be Removed)

| Package | Version | Purpose |
|---------|---------|---------|
| `firebase_core` | `^2.24.2` | Core initialization. **(DEPRECATED: Will be replaced by FastAPI)** |
| `firebase_auth` | `^4.15.3` | **(DEPRECATED: Will be replaced by JWT)** |
| `cloud_firestore` | `^4.13.6` | **(DEPRECATED: Will be replaced by PostgreSQL REST APIs)** |
| `firebase_database` | `^10.4.0` | **(DEPRECATED: Will be replaced by FastAPI WebSockets)** |

## ☁️ Future Backend & Services

| Package | Version | Purpose |
|---------|---------|---------|
| `dio` | `^5.4.3` | Powerful HTTP client used for future FastAPI integration. |
| `web_socket_channel` | `^2.4.0` | (Planned) WebSockets for 1v1 matchmaking and active quiz battles. |

## 🔐 Security & Networking

| Package | Version | Purpose |
|---------|---------|---------|
| `dio` | `^5.4.3` | Powerful HTTP client used for REST communications. |
...
| `hive` & `hive_flutter` | `^2.2.3` | High-performance local NoSQL database currently used for prototyping data persistence. |

## ♟ Game Extensions

| Package | Version | Purpose |
|---------|---------|---------|
| `flutter_chess_board` | `^1.0.1` | Used alongside Flame for specialized logic-based learning games. |
| `chess` | `^0.7.0` | Headless chess logic and validation. |
